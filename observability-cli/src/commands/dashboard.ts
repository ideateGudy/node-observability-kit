...
export async function runDashboardCommand(options: DashboardCommandOptions) {
  ...
  console.log(pc.bold(pc.cyan("\n🚀 @ideategudy/observability-cli - Dashboard Installer\n")));
...
`;
  ...
  // Next steps summary
  console.log(pc.bold(pc.green("\n🎉 Observability Dashboard successfully installed!\n")));
  console.log(pc.bold("Next steps:"));
  console.log(` 1. Install UI package:`);
  console.log(`    ${pc.cyan(`${ctx.packageManager} add @ideategudy/observability-ui lucide-react`)}`);
...
function generateDashboardPageCode(template: string, route: string, framework: string): string {
  ...
  return `// Generated by @ideategudy/observability-cli
// Route: ${route}
${isAppRouter ? '"use client";\n' : ""}
import { ${template} } from "@ideategudy/observability-ui";
