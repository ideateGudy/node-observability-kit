  ...
  return (
    <div style={{ padding: "0.5rem", background: "transparent", color: "#f8fafc", fontFamily: "Inter, system-ui, sans-serif" }}>
      <MetricGrid>
        ...
        <MetricCard title="P95 Latency" value={`${s.summary.p95LatencyMs} ms`} subtitle={`Avg: ${s.summary.avgLatencyMs}ms`} icon={<Clock size={16} color="#a855f7" />} statusColor="indigo" />
        <MetricCard title="CPU" value={`${s.runtime.cpuPercent}%`} subtitle={`RAM: ${s.runtime.memoryRssMb}MB`} icon={<Zap size={16} color="#f59e0b" />} statusColor="amber" />
      </MetricGrid>
    </div>
  );
}
